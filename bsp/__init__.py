...
class Bsp(object):
    ...
    def __call__(self, x, cond=False, cond_dims=[], cond_x=None):
        ...
        if cond:
            ...
            ndata = np.sum(self.counts[ccheck])
        else:
            ndata = self.ndata
        # Go through each domain
        for idom, domain in enumerate(domains[ccheck]):
            ...
            if check:
                return (self.counts[idom]/self.volumes[idom]) * (1/ndata)
    ...
    def _bw_sidelength(self, domain):
        # Get side lengths of domain
        ddomain = abs(np.diff(self.domains[domain]))
        # Calculate bw based off this (S.D. set to 1/3 of half dist.)
        bw = (ddomain*0.5)/3.0
        return bw
    ...
    def sample_gkde(self, N, **kw):
        ...
        if bw == "side_length":
            bw = self.bw_sidelength
        else:
            bw = lambda _: COV*bw**2.0
        # For each domain drawn
        for i, domain_draw in enumerate(domain_draws):
            ...
            perturb = scist.multivariate_normal(
                            ...
                            bw(self.domains[domain_draw])).rvs(1)
            u_draws[:, i] = np.ma.clip(ref_data + perturb,
                                        a_min=np.min(self._data,axis=1),
                                        a_max=np.max(self._data,axis=1))

            # Truncate draws to [0,1]
            

                # pdf = scist.gaussian_kde(self._data[:, I], bw_method=bw)
                # Use domain bounds and take uniform dist over the domain
